...
public class Consumer1Test {
    ...
    public void shouldReturnSuccess() throws IOException {
        ...
        Request request = Request.Get(this.greetingProvider.getConfig().url() + "/billTest/greetings");
    ...
    public void shouldReturnMemberSuccess() throws IOException {
        ...
        Request request = Request.Get(this.greetingProvider.getConfig().url() + "/billTest/greetings2?userid=88");
        HttpResponse httpResponse = request.execute().returnResponse();
        assertEquals(200, httpResponse.getStatusLine().getStatusCode());
        InputStream content = httpResponse.getEntity().getContent();
        assertEquals("Hello Bill!", IOUtils.toString(content));
